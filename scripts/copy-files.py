...
Copies files from the cape repository to a target directory.

This script copies files from the cape repository to a target directory:
...
REQUIRED_DIRS = ["workflows", "scripts"]
...
COPY_CONFIG = [
    CopyMapping("workflows", "workflows", "*", "Workflows", recursive=True),
]
...
def main(
    ...
    This script copies files for agents, hooks, commands, and optionally AI documentation
    from the cape repository to your target directory.

    Examples:

        uv run scripts/copy-files.py ~/my-project

        uv run scripts/copy-files.py /path/to/target --force

        uv run scripts/copy-files.py /path/to/target --with-envs

        uv run scripts/copy-files.py /path/to/target --no-include-ai-docs
    """
