    ...
    def execute_workflow(self, issue_id: int, description: str) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                self.logger.info(f"Successfully completed issue {issue_id}")
                self.logger.debug(f"Workflow output: {result.stdout}")
                update_issue_status(issue_id, "completed", self.logger)
                return True
            else:
                self.logger.error(
                    f"Workflow failed for issue {issue_id} with exit code {result.returncode}"
                )
