...
def worker(mock_env, worker_config):
    ...
    with patch("cape_worker.database.get_client"):
...
class TestGetNextIssue:
    ...
    def test_get_next_issue_success(self, mock_env):
        ...
        mock_client = Mock()
        mock_response = Mock()
        mock_response.data = [{"id": 123, "description": "Test issue"}]
        mock_client.rpc.return_value.execute.return_value = mock_response

        with patch("cape_worker.database.get_client", return_value=mock_client):
...
class TestExecuteWorkflow:
    ...
    def test_execute_workflow_success(self, worker):
        ...
        with patch("subprocess.run", return_value=mock_result):
            with patch("cape_worker.database.update_issue_status") as mock_update:
    ...
    def test_execute_workflow_command_format(self, worker):
        ...
        with patch("subprocess.run", return_value=mock_result) as mock_run:
            with patch("cape_worker.database.update_issue_status"):
...
class TestWorkerRun:
    ...
    def test_run_processes_issue(self, worker):
        ...
        with patch("cape_worker.database.get_next_issue", side_effect=mock_get_next_issue):
...
class TestCommandLineInterface:
    ...
    def test_main_with_required_args(self, mock_env):
        ...
        with patch("sys.argv", test_args):
            with patch("cape_worker.cli.IssueWorker") as mock_worker_class:
                mock_worker = Mock()
