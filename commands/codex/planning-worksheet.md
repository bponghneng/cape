...
**Important Reminder:** READ ONLY AND OUTPUT TWO MARKDOWN DOCUMENTS! Do not implement. Just compile the two documents and follow the `Output Format`.

## Instructions

Using the `Template A` and `Template B` below, the current chat context and any `Additional Context` submitted:

- Produce TWO completed Markdown documents:
   - Document 1: What and Why (Template A)
   - Document 2: How: Implementation Plan (Template B)
- Extract Intent, Value, and Signals of success from the conversation for Document 1. Keep them concise.
- For Document 2, outline Constraints & principles and Unknowns & assumptions.
   - Use [NEEDS CLARIFICATION: ...] for any ambiguity rather than guessing.
- Provide a brief Approach sketch aligned to the conversation context.
- Set an appropriate Execution cadence (e.g., “Generic phases” or “Add QA checkpoint prior to release”).
- Fill Logistics with any IDs/owners/links mentioned in the conversation. If not present, leave placeholders.
- If "$ARGUMENTS" is provided, incorporate them as clarifying notes or into relevant sections of either document.
- Output only the two completed Markdown documents. Do not include commentary, the raw template, or extra prose.
