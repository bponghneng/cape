...
from supabase_client import fetch_issue, create_issue as db_create_issue, create_comment, get_client
...
class WorkflowScreen(Screen):
    ...
    def run_workflow(self) -> None:
        ...
        try:
            ...
            command, error = adw_plan_build.classify_issue(issue, self.adw_id, logger)
            if error:
            ...
            plan_response = adw_plan_build.build_plan(issue, command, self.adw_id, logger)
            ...
            plan_file, error = adw_plan_build.get_plan_file(plan_response.output, self.adw_id, logger)
            if error:
            ...
            impl_response = adw_plan_build.implement_plan(plan_file, self.adw_id, logger)
