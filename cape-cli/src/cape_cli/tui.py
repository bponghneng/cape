...
from cape_cli.database import fetch_issue, create_issue as db_create_issue, create_comment, fetch_all_issues, fetch_comments, update_issue_description, delete_issue
...
class IssueListScreen(Screen):
    ...
    def action_delete_issue(self) -> None:
        ...
        row_key = table.coordinate_to_cell_key((0, table.cursor_row)).row_key
    ...
    def handle_delete_confirmation(self, issue_id: int, row_key: str, confirmed: bool) -> None:
    ...
    @work(exclusive=True, thread=True)
    def delete_issue_handler(self, issue_id: int, row_key: str) -> None:
        """Delete issue in background thread."""
        try:
            delete_issue(issue_id)
            # Update UI from thread
            self.app.call_from_thread(
                self._remove_row_and_notify,
                row_key,
                f"Issue #{issue_id} deleted successfully"
            )
        except Exception as e:
            self.app.call_from_thread(
                self.notify,
                f"Error deleting issue: {e}",
                severity="error"
            )

    def _remove_row_and_notify(self, row_key: str, message: str) -> None:
    ...
    def delete_issue_handler(self) -> None:
        ...
        try:
            ...
            self.app.call_from_thread(
                ...
                f"Issue #{self.issue_id} deleted successfully"
            )
        except Exception as e:
            self.app.call_from_thread(
                self.notify,
